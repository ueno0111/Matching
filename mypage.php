<?php
	...
	if($_GET["action"] == "user_update"){
		try {
			...
			//対象の相手のプロフィール情報を一旦削除、更新するときは一度削除をする
			$dbh = new PDO($dsn, $user, $password);
		...
		//更新したプロフィール情報を取ってくる
		try {
			$dbh = new PDO($dsn, $user, $password);	//PDO（PHP Data Object）、データベースに接続するための命令が用意されています
	...
	<body>
		<header class="gl-Header">
			<div class="gl-Header_Inner">
				...
				<section class="edit">
					...
					<button type="button" id="openModal">お相手を検索する</button>
		...
		<main>
			<form method="post" action="matching.php?action=user_login">
				...
				<section class="check-box2">
					...
					<div class="age2-text"><?php echo $result[0]['age']; ?>才</div><!--selectedという文字列が入ってきたら選択状態になる。新規登録には必要ないが、更新にはデータを保持する場合に使う-->
			...
			<div class="edit-wrapper">
				<div class="edit-container">
					<form method="post" action="mypage.php?action=user_update" enctype="multipart/form-data">
							<button type="button" id="openModal2">編集する</button>
							<div id="modalArea2" class="modalArea2">
				...
				<div class="delete-wrapper">
					<div class="delete-container">
						<button id="openModal2" onclick="delete_id(<?php echo $_SESSION["user_id"]; ?>)">退会する</button>
						<!--ダイアログ-->
						<script>
							function delete_id(user_id){
								var result = window.confirm('退会しても宜しいですか？');
						
								if( result ) {
								// formタグのもとを作成
								var _form = document.createElement("form");
								var _tmpInput = document.createElement("input");
									_tmpInput.setAttribute("name", "user_id");   // inputタグのname属性の値
									_tmpInput.setAttribute("type", "text");   // inputタグのtype属性の値（全部textで良さそうな気がする→hiddenで大丈夫のような気もする）
									_tmpInput.setAttribute("value", user_id); // inputタグのvalue属性の値
									_form.appendChild(_tmpInput);//inputタグを子要素として追加
									_form.action = "delete_id_done.php";   // formタグのaction属性の値と同じ。
		...
		<footer>
			<section class="FooterSection">
				...
				<div class="Footer-Inner-CopyRight">©2021 婚活マッチング制作</div>
			</section>
		</footer>
